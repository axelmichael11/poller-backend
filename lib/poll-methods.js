...
poll.userPollValidate = function(incomingPoll){
    let {nickname, pollQuestion, pollSubject, type, answerOptions} = incomingPoll;
    ...
    if (!poll.type || poll.type.length > 3 || typeof poll.type !== 'string'){
      throw new Error('invalid polltype type or length, or nonexistant property');
    }
    if (!poll.answerOptions || poll.answerOptions.length > 5 || typeof poll.answerOptions !== 'object'){
      throw new Error('invalid question type or length, or nonexistant property');
    }
    return poll
}
